<?php
	...
	if (isset($_GET['edit'])) {
		...
		$results2 = mysqli_query($db, "SELECT ordertype FROM details LEFT JOIN products ON products.id=details.product_id LEFT JOIN sales ON sales.id=details.sales_id WHERE details.sales_id=$id");
		$results1 = mysqli_query($db, "SELECT * FROM details LEFT JOIN products ON products.id=details.product_id LEFT JOIN sales ON sales.id=details.sales_id WHERE details.sales_id=$id");
		...
		$datetime = mysqli_query($db, "SELECT sales_date FROM details LEFT JOIN products ON products.id=details.product_id LEFT JOIN sales ON sales.id=details.sales_id WHERE details.sales_id=$id");
...
<?php endif ?>
	...
	<div class="invoice-box" >
		<div class="center"><img src="./img/favicon.png" alt="Company logo" style="width: 100%; max-width: 100px" /></div>
  ...
  <span style="float: left">Invoice ID</span>
</p> 


<p>
  
  <span style="float: left">Created</span>
</p> 
...
<span style="float: left">Address</span>
					...
					<div class="column2" style="background-color:white; text-align: left; margin-left: 5px;">
					#<?php  echo $id; ?>
...
while ($row = mysqli_fetch_array($datetime)) {
	echo date('d-m-Y h:i a', strtotime($row['sales_date']));
...
";
	...
	<tr class="heading">
				...
				<tr class="details">
					<td>Cash</td>
					<td></td>
					<td>Rm <?php echo $amount = number_format((float)$amount,2,'.','');	?></td>
				</tr>
				<tr >
					<td></td>
					<td></td>
					<td><b>Total        :</b>	Rm <?php echo $total = number_format((float)$total,2,'.','');	?></td>
				</tr>
				<tr >
					<td></td>
					<td></td>
					<td><b>Balance : </b><?php if($balance >= 0){ echo  "Rm ".number_format((float)$balance,2,'.','');}	else { echo  "<b style='color:red;'>Rm ".number_format((float)$balance,2,'.','')."<b>";}?> <?php echo $unpaid; ?></td>
				</tr>
			</table>
		</div>
		<?php 
